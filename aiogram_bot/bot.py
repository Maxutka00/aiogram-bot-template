...
from aiogram_bot import handlers, utils, web_handlers
from aiogram_bot.data import config
from aiogram_bot.middlewares import StructLoggingMiddleware, DataBaseMiddleware
...
def setup_middlewares(dp: Dispatcher) -> None:
    ...
    dp.update.middleware(DataBaseMiddleware(db=dp["db_pool"]))
...
async def aiogram_on_startup_webhook(dispatcher: Dispatcher, bot: Bot) -> None:
    ...
    webhook_logger.info(f"Configured webhook @{(await bot.me()).username}")
...
async def aiogram_on_startup_polling(dispatcher: Dispatcher, bot: Bot) -> None:
    ...
    await setup_aiogram(dispatcher)
    dispatcher["aiogram_logger"].info(f"Started polling @{(await bot.me()).username}")
